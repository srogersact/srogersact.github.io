...
window.addEventListener("load", e => {
    ...
    for (var i = 0; i < numYouTryAnalysisDivs; ++i) {
        ...
        submitButton.addEventListener("click", e => {
            ...
            feedbackElement.innerHTML = `Oops! <br>Did you find the missing word or words?<br>Did you keep everything else the same?`;
            feedbackElement.style.color = 'red';
    ...
    for (var i = 0; i < numYouTryCompositionDivs; ++i) {
        ...
        var prompt = document.createElement('p');
        prompt.innerHTML = `<b>COMPOSE: </b><br>${composition_problems[i].analyzedSentence}`;
        ...
        submitButton.addEventListener("click", e => {
            ...
            if (solution.trim() === composition_problems[targetNode.id].composedSentence) {
                feedbackElement.innerHTML = `Way to go!! <br>Now you have clauses that are easier to understand!!`;
                feedbackElement.style.color = 'green';
                return;
            }

            // check: in all other cases, prompt them to try again.
            feedbackElement.innerHTML = `Did you add in the missing word or words? <br>Did you leave everything else the same?`;
            feedbackElement.style.color = 'red';
